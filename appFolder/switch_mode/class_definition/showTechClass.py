...
class ShowTech:
    ...
    def command_processor(self, command):
        ...
        elif 'show running-config sanitized interfaces' in command:
            return(shrunint(self.command_searcher('show running-config sanitized'), command))

        elif re.search(r'show interfaces (Et|Po).* status$', command, re.IGNORECASE):
            return(individual_interface_status(self.command_searcher('show interfaces status'), command))
        
        elif re.search(r'show interfaces (Et|Po|Vx).* switchport$', command, re.IGNORECASE):
            return(interfaces_switchport(self.command_searcher('show interfaces switchport'), command))
        
        elif re.search(r'show interfaces ethernet.* mac-detail$', command, re.IGNORECASE):
            return(mac_detail(self.command_searcher('show interfaces mac-detail'), command))

        elif re.search(r'show interfaces ethernet.* phy-detail$', command, re.IGNORECASE):
            return(phy_detail(self.command_searcher('show interfaces phy-detail'), command))
        
        elif re.search(r'show interfaces (ethernet|port-channel|vlan)', command, re.IGNORECASE):
            return(individual_interfaces(self.command_searcher('show interfaces'), command))
        ...
        elif re.search('show vlan \d+', command):
            return(perVlan(content=self.command_searcher('show vlan'), command=command))
    ...
    def complete(self, text, state):
        ...
        for words in range(len(mod_ori_command) - 1):
            mod_ori_command[words] = self.sed(mod_ori_command[words])
